...
pipeline {
    ...
    environment {
        ...
        DEVELOPMENT_CONN_URL = "scm:git:https://${env.GITHUB_APIKEY}@github.com/kdavid76/sm-parent.git"
    }
    tools {
        jdk 'oracle-jdk-17'
        maven 'mvn-3.8.7'
    ...
    stages {
        ...
        stage('Push release to GitHub') {
            steps {
                sh('git add .')
                sh("git commit -m 'SM-PARENT: Release ${NEXT_RELEASE}'")
        ...
        stage('Push snapshot to GitHub') {
            steps {
                sh('git add .')
                sh("git commit -m 'SM-PARENT: Next snapshot ${NEXT_SNAPSHOT}'")
                sh('git push')
            }
        }
    }
